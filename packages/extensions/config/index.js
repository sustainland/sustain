...
function loadConfig () {
  if (fs.existsSync('sustain.yaml')) {
    return loadConfigFile('sustain.yaml')
  } else {
    let templ = {}
    multiFile = true
    ...
    for (let i = 0; i < files.length; i++) {
      if (files[i].endsWith('.yaml')) {
        let keyName = files[i].substring(0, files[i].length - '.yaml'.length)
...
function requireSettings (settings) {
  ...
  if (erredSettings.length > 1) {
    throw new Error('The following settings are required in sustain.yaml: ' + erredSettings.join('; '))
  }

  if (erredSettings.length === 1) {
    throw new Error(erredSettings[0] + ' is required in sustain.yaml')
  }
}
