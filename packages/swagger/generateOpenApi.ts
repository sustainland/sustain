import {ROUTE_ARGS_METADATA, SWAGGER_META_DATA, SWAGGER_ALLOWED_TYPES, OpenAPITypes} from './constants';
import * as pathToRegexp from 'path-to-regexp';
import {RouteParamtypes, RequestMethod} from '@sustain/http';

const yamlconfig = require('@sustain/config');
const swaggerConfiguration = yamlconfig.swagger;
...
export function generateMethodSpec(controllers: any) {
  // TODO : replace it with yaml config
        ...
        if (controllers.hasOwnProperty(method)) {
          ...
          routes.forEach((route: any) => {
            ...
            OpenApiSchema.paths[url_path][method.toLocaleLowerCase()] = {
              ...
              tags: [route.objectHanlder.constructor.prototype.API_TAG || route.objectHanlder.constructor.name],
              parameters: [...getPathParams(route), ...getRequestBody(route, method)],
...
export function expressToOpenAPIPath(expressPath: string) {
  ...
  return tokens.map((d: any) => (typeof d == OpenAPITypes.String ? d : `${d.prefix}{${d.name}}`)).join('');
}
...
function getRequestBody(route: any, method?: any) {
  ...
  for (const paramsKeys in routeArgsMetadata) {
    ...
    if (CRUD_MODEL && method && [RequestMethod.POST, RequestMethod.PUT, RequestMethod.PATCH].includes(method)) {
      useSchema = {
        $ref: `#/definitions/${CRUD_MODEL}`,
      };
    } else {
      if (!SWAGGER_ALLOWED_TYPES.map(type => type.toLocaleLowerCase()).includes(routeArgsMetadata[paramsKeys].type)) {
        useSchema = {
    ...
    if (Object.keys(properties).length !== 0) {
      ...
      if (method && (method == RequestMethod.POST || method == RequestMethod.PUT || method == RequestMethod.PATCH)) {
        tokens = [
          {
            in: 'body',
            name: 'body',
